...
class MaskOverview extends React.Component {
  constructor(props) {
    super(props);
  ...
  getTerm = async() => {
    try {
      const res = await axios.get(`/mask/term/${this.state.startDate}/${this.state.endDate}`)
  ...
  getThisMonth = async() => {
    try {
      const res = await axios.get(`mask/this-month`);
  ...
  getLastMonth = async() => {
    try {
      const res = await axios.get(`mask/last-month`);
