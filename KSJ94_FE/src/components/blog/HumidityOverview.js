...
class HumidityOverview extends React.Component {
  constructor(props) {
    super(props);
  ...
  getTerm = async() => {
    try {
      const res = await axios.get(`/humidity/term/${this.state.startDate}/${this.state.endDate}`)
  ...
  getThisMonth = async() => {
    try {
      const res = await axios.get(`humidity/this-month`);

      let date = new Array();

      for(const item of res.data) {
        // 반환 된 객체에서 humidity 값만 가져옴
        let temp = new Array(String(item.humidity))
  ...
  getLastMonth = async() => {
    try {
      const res = await axios.get(`humidity/last-month`);
