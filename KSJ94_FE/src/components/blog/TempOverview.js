...
class TempOverview extends React.Component {
  ...
  getTerm = async() => {
    try {
      const res = await axios.get(`/temperature/term/${this.state.startDate}/${this.state.endDate}`)
  ...
  getThisMonth = async() => {
    try {
      const res = await axios.get(`temperature/this-month`);

      let date = new Array();

      for(const item of res.data) {
        // 반환 된 객체에서 평균 온도 값만 가져옴
        let temp = new Array(String(item.temp))
  ...
  getLastMonth = async() => {
    try {
      const res = await axios.get(`temperature/last-month`);
